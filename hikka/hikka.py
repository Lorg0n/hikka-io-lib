...
class Hikka:
    def __init__(self, auth=None):
        self.auth = auth
        self.auth = auth if self.is_auth_code_works() else None

    def is_auth_code_works(self):
        try:
            respond = requests.post(f"{HIKKA_URL_BASE}/auth/activation/resend", headers={
                "auth": self.auth
            })
            return respond.json()["code"] == "auth:already_activated"
        except:
            return False
